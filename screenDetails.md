...
This picking list can be easily modified (add new items, change quantities). Pick-list maintenance shows every active stock item in category and item-name order, with a blank quantity where the parcel does not contain the item; a retired item remains shown only where that parcel already contains it. This generates a printable picking list page per referral, with a referral number and the stock items they require and quantities. The printed sheet remains in shelf order and shows the optional item description under its name when there is one.
...
When a pick list is created, the client applies its maintained preference-rule configuration to the referral answers and current active stock catalogue, then sends the resolved item ids with the creation request. A rule can select an item at a fixed quantity, or mark it as needing attention where the team lead must decide the quantity. A line needing attention is shown as such rather than as a negative number. It must be given a positive quantity or removed before that parcel can be reviewed, so it cannot be printed or issued. Administrators have a Preference rule check screen which validates the configured question keys, choice values and stock names against the current environment.

Once all pick lists have been generated and every parcel has been reviewed there is an option to print all of them. Printing is unavailable until then, so a parcel still needing a team leader's decision cannot reach paper. The session client list keeps each pick number with the name, followed by a status: Pending Review, Pick List reviewed, Attended/Delivered, or No Show/Not in. A pending client has a Review Pick list link. Once reviewed, Attended/Delivered and No Show/Not in become clickable. A Complete Session button sits above the list and is available only after every client has an outcome. Selecting a client opens a separate client workspace, so the session client list does not compete for screen space. Its heading shows the pick number and client name, with Adults/children right justified. The client workspace has a prominent Mark Pick List reviewed button at the top, but its attendance and Complete Session controls remain on the client list. The pick list and the client’s preferences are shown in separate, independently scrollable panels, each taking at least one third of the screen. Category change headers are smaller and lighter than stock-item names. Preferences use the referral field key and the answer in aligned columns; the full question is not repeated. Staff can alter several quantities and add items while considering the preferences, then save all those changes with one Save action. If they return to the client list, print, record attendance or complete the session while changes have not been saved, the system asks whether to save first. To confirm attendance (or delivery to) a client (or to confirm non attendance - one or the other must be done)
...
The referral list shows the ones waiting first, and marks each pending-review line as needing attention, because a household waiting on a decision is more urgent than one already booked in.
...
Administrators have a separate referral-search screen, reached from the referrals menu. It accepts a postcode, a phone number and/or a date of birth; when more than one is supplied, a referral matching any one is shown. Each result shows the client name, address, session date and location, status, and which supplied identifier matched. It is an administrator-only search because these identifiers and its results are personal information.

Every referral also waits for an administrator to read it through. A referral from a recognised address starts Active; once it is read, it becomes Reviewed without changing its booking or parcel. The detail screen offers an administrator a Mark reviewed button at both the top and bottom while the referral is Active. The referral list highlights Active rows so that the referrals still to be read stand out from Reviewed ones.

The referral detail screen shows its header information, then the previous possible referrals, then the approval/rejection and cancellation controls. Its fixed fields are read-only until an administrator chooses Edit details; saving is offered only in that editing state. Moving to another session follows the fixed fields, and the other answers from the referral form come last. The reason-for-referral and session-to-move controls are large enough to read and operate comfortably.
...
#Listener sheet
From a Run a session screen, a team leader or administrator can open and print one listener sheet for that session. Like Print all pick lists, the link is on the selected-session screen, but unlike it the listener sheet is available regardless of whether pick lists have been created or reviewed. It is a printed page for the dedicated trained volunteers who discuss the household's situation with clients and offer guidance where appropriate. It includes every non-delivery household except cancelled and rejected referrals. The sheet contains only the household name, other information, reason for crisis, secondary reason and the fuel-help flag. It never contains an address, postcode, phone number, referrer information, date of birth, parcel contents or any other form answer. It is the only printed document on which a team leader may see the reason for referral, and is for the selected listeners only.

#Fuel help list
A fuel administrator signs into one screen: the fuel help list. Administrators can also open it; team leaders cannot. It is a plain table intended to be copied into Excel, with one row for every referral returned by the server. It shows the parcel session date, name, address, postcode, phone number, pre-payment-meter answer and permission-to-ring answer. Permission to ring is prominent and does not filter anybody out: the person doing the fuel work decides whether to call. The screen never shows a reason for referral, date of birth, household counts, delivery status or any answer other than those two fuel questions. Its introductory text does not tell the user how to interpret the permission answer.
